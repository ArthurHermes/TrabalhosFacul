...
public class FloodFill {
    public static void main(String[] args) {
        int contador = 0;
        int contadorNome = 0;
        File arquivo = new File("Estrutura_de_Dados\\FloodFill\\FloodFillFila\\Marina.png");
        if (!arquivo.exists()) {
        ...
        try {
            ...
            fila.offer(new int[]{xInicio, yInicio});

            File diretorioSaida = new File("Estrutura_de_Dados\\FloodFill\\FloodFillFila\\out");
            ...
            while (!fila.isEmpty()) {
                ...
                if (corSimilar(corAtual, corAlvo) && !corSimilar(corAtual, corFronteira)) {
                    ...
                    fila.offer(new int[]{x + 1, y});
                    fila.offer(new int[]{x - 1, y});
                    fila.offer(new int[]{x, y + 1});
                    fila.offer(new int[]{x, y - 1});
                }

                if (contador % 500 == 0) {
                    File arquivoSaida = new File("Estrutura_de_Dados\\FloodFill\\FloodFillFila\\out/" + contadorNome + ".png");
                    ImageIO.write(novaImagem, "png", arquivoSaida);
                    contadorNome++;
                }
            }

            File arquivoFinal = new File("Estrutura_de_Dados\\FloodFill\\FloodFillFila\\out/quadrado_rosa_filled.png");
